...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Lockout check
    const { data: attemptsCount, error: countErr } = await supabase.rpc(
    ...
    if (countErr) {
      ...
      return jsonResponse({ error: 'Erreur serveur (lockout)' }, 500, origin);
    }
    ...
    if (verifyErr) {
      ...
      return jsonResponse({ error: 'Erreur serveur (verify)' }, 500, origin);
    }

    const row = Array.isArray(verifyData) && verifyData.length > 0 ? verifyData[0] : null;

    // Log attempt (success or fail)
    const { error: logErr } = await supabase.from('login_attempts').insert({
    ...
    if (logErr) console.error('login_attempts insert error', logErr);
    ...
    console.error('login-admin unhandled error', err);
    return jsonResponse(
